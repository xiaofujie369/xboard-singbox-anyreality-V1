...
if docker exec sing-box grpcurl -plaintext -import-path /usr/local/share/sing-box -proto stats.proto -d '{"pattern":"user>>>","reset":false}' 127.0.0.1:8080 v2ray.core.app.stats.command.StatsService/QueryStats >/dev/null 2>&1; then echo "[OK] Stats API"; else echo "[FAIL] Stats API"; failures=$((failures+1)); fi
